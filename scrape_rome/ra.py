...
clubs = { # ids must be strings to performa the request correctly
...
df = df[['startTime', 'title', 'venue_name', 'contentUrl']].copy()
df['startTime'] = pd.to_datetime(df['startTime']).dt.strftime('%Y-%m-%d %H:%M:%S')
df['contentUrl'] = 'https://ra.co' + df['contentUrl']
df.rename(columns={'startTime': 'date_and_time','title': 'name', 'venue_name': 'location', 'contentUrl': 'url'}, inplace=True)
df.to_csv('../data/ra_test.csv', index=False)